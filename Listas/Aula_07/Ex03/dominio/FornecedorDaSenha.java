...
public class FornecedorDaSenha implements Jogador {
	 ...
	 * executada uma única vez que é quando o jogo começa. Quem manda essa função 
	 * ser executada é a mesma classe que cria uma instância de FornecedorDaSenha: "Jogo".
	 * A função segue os seguintes passos: 
	 *  1 - Armazenar em uma LinkedList várias Strings. Cada String será uma cor
	 *      válida usada pelo sistema (vermelho, azul, rosa, amarelo, roxo, verde,
	 *      cinza ou laranja). 
	 *  2 - Retirar 4 cores aleatórias da lista, sem repetição. 
	 *  3 - Ir adicionando na "senha"(atributo do FornecedorDaSenha) essas 4 cores.
	 */
	...
	/**
	 * A função "jogar()" do FornecedorDaSenha realiza os seguintes passos: 
	 * 1 - Pegar a tentativa do Adivinho. A tentativa pode ser obtida através do
	 *     objeto "jogada" que o Fornecedor e o Adivinho compartilham. 
	 * 2 - Para cada pino da tentativa, será checado se ele é da mesma cor que um 
	 *     pino da "senha" e se ele está na mesma posição que a senha. 
	 *  2.1 - Caso um pino da tentativa seja da mesma cor e esteja na mesma posição 
	 *        que um pino da senha, um novo pino preto é adicionado ao "retorno" (resultado 
	 *        da comparação entre a Senha e a Tentativa) gerado pelo Fornecedor. 
	 *  2.2 - Caso um pino da tentativa seja da mesma cor que algum pino da senha mas
	 *        os dois não estão na mesma posição, um novo pino branco é adicionado ao
	 *        "retorno" 
	 *  2.3 - Se o pino presente na tentativa do Adivinho não está
	 *        presente na senha gerada pelo Fornecedor, nenhum novo pino é adicionado
	 *        ao "retorno". 
	 * 3 - O passo 2 será repetido para cada pino da tentativa e no final o retorno 
	 *     poderá ser acessado por qualquer um que tenha acesso à Jogada (objeto compartilhado 
	 *     entre Adivinho e Fornecedor e criado pela classe "Jogo").
	 */
	public void jogar() {
